			...
			<view class="cu-list menu text-left padding-top-xs">
				<view class="cu-item" v-for="item in searchData" :key="item.org_id">
					<view class="content">
						...
						<text class="cuIcon-close text-gray" style="float: right;font-size: 40upx;margin-right: 0upx; width: 1.2em;" @tap="delFile(item)"></text>
					</view>
				</view>
			</view>
		</view>
			...
			uploadFile(e) {
				...
				if(platform == 'ios'){ 
					...
					fileSelect.openFile({
						...
						'size': '1', //选择总数量
						'type': ['pdf','word','ppt','xls'] //image,xls,word,ppt,pdf,mp3,mp4,zip,rar  不填默认全部
					}, (ret) => {
						...
						global.$http.upload('/oos/upload', {
							...
							uni.hideLoading();
							if (res.status === "0") {
								this.para.file_name = ret.data[0].name;
