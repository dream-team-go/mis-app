<template>
	<view>
		<cu-custom bgColor="bg-linear-blue" :isBack="true">
			<block slot="content">会议汇总展示</block>
		</cu-custom>
			...
			<view class="cu-item" style="height: 132upx;" v-for="record in records" :key="record.id" @click="recordDetail(record)">
				...
				<view class="content" style="left: 118upx;width: calc(100% - 118upx - 10upx - 110upx);">
					<view class="text-black">
						<text class="text-df">{{getWeekDay(record.ydrq)}} {{record.ydsjd == "1" ? '上午' : '下午'}}{{record.start_time.substring(11,16)}}</text>
...
<script>
	...
	export default {
		...
		methods: {
			loadData() {
				this.status = 'loading';
				global.$http.post('/meeting/record/meetingRecordQk', {
					params: {
			...
			getStatusDesc: function(e){
				return misEnum.MeetingRecordEnumMap.get(e.status);
			},
