<template>
	<view>
		<cu-custom bgColor="bg-linear-blue" :isBack="true">
			<block slot="content">接待汇总展示</block>
		</cu-custom>

		<view class="cu-list menu-avatar">
			<view class="cu-item" style="height: 132upx;" v-for="record in records" :key="record.id" @click="recordDetail(record)">
				<view class="cu-avatar round lg bg-linear-blue" style="left: 12upx;"><text class="text-lg text-bold">{{getDateStr(record.jd_date)}}</text></view>
				<view class="content" style="left: 118upx;width: calc(100% - 118upx - 10upx - 110upx);">
					<view class="text-black">
						<text class="text-df">{{record.jd_object}}</text>
					</view>
...
<script>
	...
	export default {
		...
		methods: {
			loadData() {
				this.status = 'loading';
				global.$http.post('/reception/receptionQk', {
					params: {
			...
			getStatusDesc: function(e){
				return misEnum.ReceptionStatusEnumMap.get(e.status);
			},
